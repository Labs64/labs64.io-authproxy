...
- **Cedar edge tier (RFC-05 P2)**: `CEDAR_MODE=off|shadow|enforce` (default `shadow`). Requires bundle mode (`POLICY_BUNDLE_DIR`); the generated per-operation Cedar policies (`modules/<name>.cedar`) ride the signed bundle. `shadow` logs `cedar-shadow ... match=True|False` per module-route request without changing behavior; `enforce` makes Cedar the decision for module routes (legacy public/tenant/scope logic retired there — static prefixes stay legacy). Engine errors and unknown operations fail closed.
...
| `CEDAR_MODE` | `shadow` | Cedar edge tier: `off` / `shadow` (log diff) / `enforce` (Cedar decides module routes) |
