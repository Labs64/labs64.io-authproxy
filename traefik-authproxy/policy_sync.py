...
namespace, fetches each module's generated Tier-1 edge Cedar policy set from
/.well-known/auth-policy.cedar in-cluster, and feeds the PolicyStore. This is
the single fetch for both the routing table (the @path/@method/@public/
@tenantRequired/@scopes annotations OpenApiAuthPreprocessor stamps on every
permit — see policy_store.parse_cedar_document) and the Cedar edge PDP's
decision input (combined_cedar()); auth-policy.json is no longer consulted on
this path (it remains the signed-bundle source, see policy_bundle.py).
Per-module failures keep the last-known-good routes/policies; a module whose
Service disappears is dropped. Readiness = the first refresh pass has
completed (success or not), so a cold-started ACS never serves before it has
at least attempted to build its table.

Modules are keyed by the first segment of their gateway base path (e.g.
/payment-gateway/api/v1 -> ``payment-gateway``), which is the same module
name the build bakes into the Cedar resource ids and bundle-sources.yaml
uses — so the live-discovery and signed-bundle paths agree on identity.
"""
import logging
import os
import threading
...
from policy_store import PolicyStore, parse_cedar_document
...
class PolicySync:
    def __init__(self, store: PolicyStore, namespace: Optional[str] = None,
        ...
        # Generated Tier-1 edge Cedar policy text per module, fetched from
        # /.well-known/auth-policy.cedar. This is also the routing source
        # (see refresh_once) so, unlike the pre-merge design, every module
        # labeled for auth-policy MUST serve it — there is no "older starter,
        # no cedar" fallback left on this path.
    ...
    def refresh_once(self) -> Dict[str, str]:
        ...
        for svc in services:
            ...
            url = f"http://{svc_name}.{self.namespace}.svc.cluster.local:{port}{WELL_KNOWN_CEDAR_PATH}"
            try:
                response = requests.get(url, timeout=self.fetch_timeout)
                response.raise_for_status()
                text = response.text
                validate_policy_text(text)
                routes = parse_cedar_document(module, base_path, text)
            except ValueError as e:  # PolicyValidationError subclasses ValueError
                logger.error("Rejected cedar policy from %s: %s — keeping last known good",
                             module, e)
